        ...
        <section className="project-header shell">
          <div className="project-copy">
            ...
            <div className="project-actions">
              <a className="primary-button" href="#live-analysis">Upload PCAP</a>
              <button type="button" className="primary-button" onClick={startDemo}><Play size={16} fill="currentColor" />Run Demo Analysis</button>
              <Link className="secondary-link" href="/analysis" onClick={()=>live.setMode("demo")}>View sample results <ArrowRight size={16} /></Link>
            </div>
          ...
          <aside className="project-run-card" aria-label="Project implementation summary">
            <div className="run-card-head"><span>DEMO INPUT — SIMULATED</span><b>campus_edge_sample.pcap</b></div>
            <code>netguard analyze input.pcap --report findings.json --filtered filtered.pcap</code>
            <div className="run-card-results"><div><strong>{demoSummary.inputPackets}</strong><span>sample packets</span></div><div><strong>{demoSummary.activeFlows}</strong><span>sample flows</span></div><div><strong>{demoSummary.alerts}</strong><span>sample alerts</span></div></div>
            <p>Demo figures are illustrative. Upload a capture below for actual engine results. Build and test status must be checked in your environment.</p>
          </aside>
        </section>
        <LiveAnalysis />
        <section id="demo" className="demo-section shell">
          <div className="demo-heading"><div><span className="section-kicker">INTERACTIVE DEMO — SIMULATED</span><h2>{isComplete ? "Analysis complete" : isRunning ? analysisPhases[runState.phaseIndex].label : "Run the included PCAP sample"}</h2><p>{isComplete ? "The sample output is ready. Use the pages below to inspect alerts, architecture, and JSON evidence." : isRunning ? analysisPhases[runState.phaseIndex].detail : "This simulates the project CLI processing a deterministic test capture."}</p></div><button type="button" className="primary-button compact" onClick={startDemo} disabled={isRunning}>{isRunning ? <><TimerReset size={16} /> Running…</> : <><Play size={16} fill="currentColor" />{isComplete ? "Run Again" : "Run Demo Analysis"}</>}</button></div>
          <div className="demo-panel">
            <div className="demo-progress"><div className="analysis-eyebrow" role="status" aria-live="polite"><Activity size={15} className={isRunning ? "active-icon" : ""} /> {isComplete ? "Completed" : isRunning ? "Processing capture" : "Ready"}</div><div className="progress-caption"><span>Simulated progress</span><strong>{isComplete ? 100 : demoRun.progress}%</strong></div><div className="progress-track" role="progressbar" aria-label="Demo analysis progress" aria-valuemin={0} aria-valuemax={100} aria-valuenow={isComplete ? 100 : demoRun.progress}><span style={{ width: `${isComplete ? 100 : demoRun.progress}%` }} /></div></div>
